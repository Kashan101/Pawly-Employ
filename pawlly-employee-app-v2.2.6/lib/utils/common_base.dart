...
List<LanguageDataModel> languageList() {
  return [
    LanguageDataModel(
        id: 1, name: 'English', languageCode: 'en', fullLanguageCode: 'en-US', flag: Assets.flagsIcUs),
    LanguageDataModel(
        id: 2, name: 'Hindi', languageCode: 'hi', fullLanguageCode: 'hi-IN', flag: Assets.flagsIcIn),
    LanguageDataModel(
        id: 3, name: 'Arabic', languageCode: 'ar', fullLanguageCode: 'ar-AR', flag: Assets.flagsIcAr),
    LanguageDataModel(
        id: 4, name: 'French', languageCode: 'fr', fullLanguageCode: 'fr-FR', flag: Assets.flagsIcFr),
    LanguageDataModel(
        id: 4, name: 'German', languageCode: 'de', fullLanguageCode: 'de-DE', flag: Assets.flagsIcDe),
  ];
}
...
TextStyle get appButtonTextStyleGray => secondaryTextStyle(
    ...
    size: 14,
    fontFamily: GoogleFonts.beVietnamPro(fontWeight: FontWeight.w500).fontFamily);

TextStyle get appButtonTextStyleWhite => secondaryTextStyle(
    color: Colors.white,
    size: 14,
    fontFamily: GoogleFonts.beVietnamPro(fontWeight: FontWeight.w600).fontFamily);

TextStyle get appButtonPrimaryColorText => secondaryTextStyle(
    color: primaryColor, fontFamily: GoogleFonts.beVietnamPro(fontWeight: FontWeight.w500).fontFamily);

TextStyle get appButtonFontColorText => secondaryTextStyle(
    color: Colors.grey, fontFamily: GoogleFonts.beVietnamPro(fontWeight: FontWeight.w500).fontFamily);
...
InputDecoration inputDecoration(BuildContext context,
  ...
  return InputDecoration(
    ...
    hintStyle: secondaryTextStyle(
        size: 12, fontFamily: GoogleFonts.beVietnamPro(fontWeight: FontWeight.w300).fontFamily),
...
void handleRate() async {
  if (isAndroid) {
    if (getStringAsync(APP_PLAY_STORE_URL).isNotEmpty) {
      ...
      commonLaunchUrl('${getSocialMediaLink(LinkProvider.PLAY_STORE)}${await getPackageName()}',
    ...
    builder: (_) {
      return PopScope(
        ...
        child: NewUpdateDialog(
            canClose: currentAppVersionCode >= appConfigs.value.employeeMinimumForceUpdateCode),
      );
    },
  );
}
...
Future<void> showForceUpdateDialog(BuildContext context) async {
  getPackageInfo().then((value) {
    if (isAndroid &&
        appConfigs.value.employeeLatestVersionUpdateCode > value.versionCode.validate().toInt()) {
...
Widget buildIconWidget(
    {required String icon, required VoidCallback onTap, Color? iconColor, double? height, double? weight}) {
  return SizedBox(
    height: height ?? 38,
    width: weight ?? 38,
    child: IconButton(
        padding: EdgeInsets.zero, icon: icon.iconImage(size: 18, color: iconColor), onPressed: onTap),
  );
}
...
String formatDate(String? dateTime,
  ...
  final languageCode = isLanguageNeeded
...
Widget detailWidgetPrice(
    {required String title, required num value, Color? textColor, bool isSemiBoldText = false}) {
  return Row(
    mainAxisAlignment: MainAxisAlignment.spaceBetween,
    children: [
